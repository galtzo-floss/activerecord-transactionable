...
in `.structuredmerge/kettle-jem.yml`; this project was templated with `ruby.test_minimum: 2.5.0`.
...
- One spec file per class/module. For each class or module under `lib/`, keep all of its unit tests in a single spec file under `spec/` that mirrors the path and file name exactly: `lib/activerecord/transactionable/my_class.rb` -> `spec/activerecord/transactionable/my_class_spec.rb`.
...
[🧪build]: https://github.com/galtzo-floss/activerecord-transactionable/actions
